...
export class SimpleWalletAdapter extends BaseWalletAdapter {
   ...
   get readyState(): WalletReadyState {
    return this._readyState;  // This ensures the wallet is treated as available and listed in the main screen
}
    ...
    private createOverlayIframe(): void {
        ...
        this.overlayIframe.style.width = '100vw';
        this.overlayIframe.style.height = '100vh';
        this.overlayIframe.style.zIndex = '9999';
        this.overlayIframe.style.border = 'none';
        document.body.appendChild(this.overlayIframe);

        const iframeDoc = this.overlayIframe.contentDocument;
        if (iframeDoc) {
            iframeDoc.open();
            iframeDoc.write(overlayHtml);
            iframeDoc.close();

            iframeDoc.getElementById('cancel-login')?.addEventListener('click', () => this.cleanup(true));
        }
    }
    ...
    private cleanup(forceClose: boolean): void {
        if (this.popup && forceClose) {
            this.popup.close();
        }
        this.popup = null;
